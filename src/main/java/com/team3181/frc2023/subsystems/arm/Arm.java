...
package com.team3181.frc2023.subsystems.arm;
...
public class Arm extends SubsystemBase {
    ...
    LazySparkMax shoulderJoint;
    LazySparkMax shoulderSecondary;
    LazySparkMax elbowJoint;
    ...
    public Arm() {
        shoulderJoint = new LazySparkMax(Constants.FourBarConstants.CAN_SHOULDER_MASTER, CANSparkMax.IdleMode.kBrake, 80, false, false);
        shoulderSecondary = new LazySparkMax(Constants.FourBarConstants.CAN_SHOULDER_FOLLOWER, CANSparkMax.IdleMode.kBrake, 80, shoulderJoint, false, true);
        shoulderEncoder = shoulderJoint.getAbsoluteEncoder(SparkMaxAbsoluteEncoder.Type.kDutyCycle);

        shoulderEncoder.setInverted(true);
        shoulderEncoder.setPositionConversionFactor(2 * Math.PI * Constants.FourBarConstants.CHAIN_RATIO);
        shoulderEncoder.setVelocityConversionFactor(2 * Math.PI * Constants.FourBarConstants.CHAIN_RATIO / 60.0);
        shoulderEncoder.setZeroOffset(Constants.FourBarConstants.SHOULDER_ABSOLUTE_OFFSET.getRadians());
        shoulderJoint.setPeriodicFramePeriod(CANSparkMaxLowLevel.PeriodicFrame.kStatus5, 20);

        elbowJoint = new LazySparkMax(Constants.FourBarConstants.CAN_ELBOW, CANSparkMax.IdleMode.kBrake, 80, false,false);
        elbowEncoder = elbowJoint.getAbsoluteEncoder(SparkMaxAbsoluteEncoder.Type.kDutyCycle);
        elbowEncoder.setInverted(true);
        elbowEncoder.setPositionConversionFactor(2 * Math.PI * Constants.FourBarConstants.CHAIN_RATIO);
        elbowEncoder.setVelocityConversionFactor(2 * Math.PI * Constants.FourBarConstants.CHAIN_RATIO / 60.0);
        elbowEncoder.setZeroOffset(Constants.FourBarConstants.ELBOW_ABSOLUTE_OFFSET.getRadians());
        elbowJoint.setPeriodicFramePeriod(CANSparkMaxLowLevel.PeriodicFrame.kStatus5, 20);
        ...
        // Here for a reason but not sure why
...
//                .degreesToFalcon(Units.radiansToDegrees(constArm.ELBOW_REVERSE_LIMIT), constArm.ELBOW_GEAR_RATIO);
    ...
    private void setShoulderPosition(Rotation2d position) {
        ...
        shoulderJoint.set(counts, ControlMode.Position);
    }
